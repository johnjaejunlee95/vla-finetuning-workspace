...
BENCHMARK_MAPPING = {}
TASK_CLASSIFICATION_FILE = os.path.join(os.path.dirname(__file__), "task_classification.json")
TASK_CLASSIFICATIONS = None
...
def get_task_classifications():
    global TASK_CLASSIFICATIONS

    if TASK_CLASSIFICATIONS is None:
        with open(TASK_CLASSIFICATION_FILE, "r") as f:
            TASK_CLASSIFICATIONS = json.load(f)
    return TASK_CLASSIFICATIONS


def get_suite_categories(suite_name):
    classifications = get_task_classifications()
    if suite_name not in classifications:
        return []

    return sorted({task_info["category"] for task_info in classifications[suite_name]})
...
class Benchmark(abc.ABC):
    ...
    def __init__(self, task_order_index=0, task_categories: Optional[List[str]] = None):
        self.task_embs = None
        self.task_order_index = task_order_index
        self.task_categories = task_categories
        self.task_metadata_by_name = {}
    ...
    def _make_benchmark(self):
        ...
        self.task_metadata_by_name = {
            ...
            for task_info in get_task_classifications().get(self.name, [])
        }
        if self.task_categories:
            self.filter_tasks_by_categories(self.task_categories)
        self.n_tasks = len(self.tasks)

    def get_available_categories(self):
        return get_suite_categories(self.name)

    def get_task_metadata(self, i):
        return self.task_metadata_by_name.get(self.tasks[i].name, {})

    def get_task_category(self, i):
        return self.get_task_metadata(i).get("category")

    def filter_tasks_by_categories(self, categories):
        categories = [category.strip() for category in categories if category.strip()]
        available_categories = self.get_available_categories()
        invalid_categories = sorted(set(categories) - set(available_categories))
        if invalid_categories:
            raise ValueError(
                f"Unknown categories for {self.name}: {invalid_categories}. "
        ...
        self.tasks = [
            ...
            if self.task_metadata_by_name.get(task.name, {}).get("category") in selected_categories
        ]
        self.n_tasks = len(self.tasks)
...
class LIBERO_SPATIAL(Benchmark):
    def __init__(self, task_order_index=0, task_categories=None):
        super().__init__(task_order_index=task_order_index, task_categories=task_categories)

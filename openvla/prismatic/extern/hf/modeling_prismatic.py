...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def predict_action(self, input_ids: Optional[torch.LongTensor] = None, unnorm_key: Optional[str] = None, **kwargs: str):
        ...
        # logits = torch.stack(generated_ids.scores, dim=1).cpu().float()
...
class CustomOpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    config_class: PretrainedConfig = OpenVLAConfig
    ...
    def predict_action(self, input_ids = None, input_pixels = None, unnorm_key: Optional[str] = None, **kwargs: str):
        """Thin wrapper around .generate() that decodes predicted actions and unnormalizes them."""
        # If the special empty token ('') does not already appear after the colon (':') token in the prompt
        # (after "OUT:" or "ASSISTANT:"), insert it to match the inputs seen at training time
        if not torch.all(input_ids[:, -1] == 29871):
        ...
        # Run VLA inference
        generated_outputs = self.generate(input_ids, pixel_values=input_pixels, max_new_tokens=self.get_action_dim(unnorm_key), output_scores=True, return_dict_in_generate=True, **kwargs)
        logits = torch.stack(generated_outputs.scores, dim=1).cpu()

...
def main(config: _config.TrainConfig):
    ...
    overall_steps = config.num_train_steps - start_step
    for step in range(start_step, config.num_train_steps):
        ...
        if completed_step % config.log_interval == 0 or completed_step == config.num_train_steps:
            ...
            steps_done = completed_step - start_step
            ...
            logging.info(
                f"Step {completed_step}/{config.num_train_steps}: {info_str}, "
                f"ETA={eta_days}d {eta_hours}h {eta_minutes}m left ({eta_time / 3_600:.2f}h)"
            )
            wandb.log(reduced_info, step=completed_step)
            infos = []
        batch = next(data_iter)

        if ((step + 1) % config.save_interval == 0 and step > start_step) or (step == config.num_train_steps - 1):
